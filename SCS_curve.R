SCS_curve <- function(cur_LU,cur_P,rain_5,CN_List){
  #Get the curve number based on LU
  cur_CN=CN_List[[as.character(cur_LU)]]
  ...
  if(cur_P>0 & cur_P>(0.2*S_AMC)){
     Qr=(cur_rain-0.2*S_AMC)^2/(cur_rain+0.8*S_AMC)
  } else {Qr=0}
